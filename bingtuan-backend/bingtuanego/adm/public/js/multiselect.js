		...
		Multiselect.prototype = {
			...
			events: function( actions ) {
				...
				self.left.on('dblclick', 'option', function(e) {
					e.preventDefault();
					var activity_goods_money = $('#activity_goods_money').val();
					if (activity_goods_money <= 0 || isNaN(activity_goods_money)) {
						alert('请输入活动价格');
					} else {
						this.text = this.text+', ￥'+activity_goods_money;
						var val = $("#result").val();
						if (val) {
							val = val+'-'+this.value+':'+activity_goods_money;
						} else {
							val = this.value+':'+activity_goods_money;
						}
						$("#result").val(val);
				...
				self.right.on('dblclick', 'option', function(e) {
					...
					delArray[0] = this.value;
					
					var val = $("#result");
					var valArr = val.val().split('-');
					for (var i=0; i<valArr.length; i++) {
						var v = valArr[i].split(':');
						if ( $.inArray(v[0], delArray) != '-1') {
							valArr.splice(i,1);
							i--;
						}
					}
					...
					if (activity_goods_money <= 0 || isNaN(activity_goods_money)) {
						...
						if ( options ) {
							options.each(function(i,option) {
								option.text=option.text+', ￥'+activity_goods_money;
								var val = $("#result").val();
								if (val) {
									val = val+'-'+option.value+':'+activity_goods_money;
								} else {
									val = option.value+':'+activity_goods_money;
								}
								$("#result").val(val);
