	...
	$.fn.extend({
		SimpleTree:function(options){
			...
			this.find("a").click(function(){
				 ...
				 * 否则修改其 show 属性为 true
				 */
				if($(this).parent().next("ul").attr("show")=="true"){
					$(this).parent().next("ul").attr("show","false");
				}else{
					$(this).parent().next("ul").attr("show","true");
				}
			...
			$(this).find(":checkbox").click(function(){
				if($(this).prop('checked') == false){
					...
					$(this).parent().next('ul').find("input[type='checkbox']").removeProp('checked');
				}else{
