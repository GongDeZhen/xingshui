            ...
            if($this->isPost('sub')){
	            if($result['success'] == 0){
	               echo "<script>alert('".$result['error']."')</script>";
	                ...
	                setcookie('receiver',$result['data']['user_type'],time() + (3600 * 24 * 7),'/');//用户类型
	                setcookie('auditType',$result['data']['auditType'],time() + (3600 * 24 * 7),'/');//用户状态
