...
class Push {
	...
	function __construct($key='', $secret='') {
	    if (!$key){
// 	        $key = Yaf_Application::app()->getConfig()->get('umeng')->get('appkey');//self::$_defaultConfig['appKey'];
	    }
	    if (!$secret){
// 	        $secret = Yaf_Application::app()->getConfig()->get('umeng')->get('appMasterSecret');//self::$_defaultConfig['appMasterSecret'];
	    }
        $this->appkey = $key;
	...
	function sendAndroidUnicast($appkey, $appMasterSecret, $device_tokens, $ticker, $title, $text) {
		try {
...
// 			print("Caught exception: " . $e->getMessage());
	...
	function sendIOSUnicast($appkey, $appMasterSecret, $device_tokens, $alert) {
		try {
